...
function StagePage(props: {
  ...
  const canPublish = operator.demoMode === "rehearsal" || operator.isConnected;
    ...
    <main className="stage-page stage-page-v2">
        ...
        <div className="stage-actions onstage-actions">
          ...
          <div className="topbar-actions">
            <div className="mode-toggle">
              <button
                className={`mode-toggle-option ${operator.demoMode === "rehearsal" ? "active" : ""}`}
                type="button"
                onClick={() => operator.setDemoMode("rehearsal")}
              >
                Rehearsal
              </button>
              <button
                className={`mode-toggle-option ${operator.demoMode === "live" ? "active" : ""}`}
                type="button"
                onClick={() => operator.setDemoMode("live")}
              >
                Live
              </button>
            </div>
            {operator.demoMode === "live" ? (
              <button className="button primary" type="button" onClick={() => operator.setIsConnected((current) => !current)}>
                {operator.isConnected ? "Disconnect relays" : "Connect relays"}
              </button>
            ) : null}
      ...
      <section className="stage-shell onstage-shell">
        <article className="onstage-hero">
          <div>
            <span className={`perspective-badge perspective-${activeStory.perspective}`}>{activeStory.perspectiveLabel}</span>
          ...
          <div className="onstage-status-row">
            {operator.demoMode === "rehearsal" ? (
              <span className="pill warn">Rehearsal — local only</span>
            ) : (
              <>
          ...
          <section className="onstage-grid">
            <div className="onstage-main">
              <article className="stage-card stage-card--primary">
                ...
                <p>Choose a handle, protect the backup, then publish the migration authority and timestamp proof.</p>
                <label className="field compact">
                  <span>Handle</span>
                  <input
                    className="input"
              ...
              <StateTimeline steps={getPreparedMigrationStateSteps(operator.pathAState)} />
            ...
            <div className="onstage-side">
              ...
              />
              <StageEventStatusCard title="Authority + Proof" rows={signupEventRows} rehearsal={operator.demoMode === "rehearsal"} />
        ...
        {activeStoryId === "happy" && currentPackage.lane === "path-a" ? (
          <section className="onstage-grid">
              ...
              <article className="stage-card stage-card--primary">
                ...
                <div className="button-stack">
                  <button
                    className="button primary"
                    type="button"
                    disabled={!canPublish || !operator.nextPreparedAction}
                    onClick={() => {
                  ...
                  <button
                    ...
                    type="button"
                    disabled={!canPublish || operator.publishCursor >= currentPackage.preparedActions.length}
                    onClick={() => {
                  ...
                  >
                    Publish all remaining ({Math.max(currentPackage.preparedActions.length - operator.publishCursor, 0)} remaining)
                  </button>
                </div>
              </article>
            ...
            <div className="onstage-side">
              ...
              />
              <StageEventStatusCard title="Event progress" rows={pathAEventRows} rehearsal={operator.demoMode === "rehearsal"} />
              <StageEventsCard
                title={operator.demoMode === "rehearsal" ? "Resolved locally" : "Now on relays"}
                events={operator.relevantObservedEvents}
                emptyMessage={operator.demoMode === "rehearsal" ? "Publish events to see them here." : "Connect, send a note, then publish the prepared events."}
              />
            </div>
          </section>
        ) : null}
          ...
          <section className="onstage-grid">
            ...
            <div className="onstage-side">
              ...
              <StageEventsCard
                title={operator.demoMode === "rehearsal" ? "Resolved locally" : "Now on relays"}
                events={operator.relevantObservedEvents}
                emptyMessage="Publish events to see them here."
              />
            </div>
          </section>
        ) : null}
        ...
        {activeStoryId === "social" && currentPackage.lane === "path-c" ? (
          <section className="onstage-grid">
            <div className="onstage-main">
              ...
              <StateTimeline steps={getSocialStateSteps(operator.pathCStateText)} />
            ...
            <div className="onstage-side">
              ...
              />
              <StageEventStatusCard title="Event progress" rows={pathCEventRows} rehearsal={operator.demoMode === "rehearsal"} />
              <StageEventsCard
                title={operator.demoMode === "rehearsal" ? "Resolved locally" : "Now on relays"}
                events={operator.relevantObservedEvents}
                emptyMessage="Publish transition claims and attestations to light this up."
              />
            </div>
          </section>
        ) : null}
        ...
        {activeStoryId === "contested" && replayPlayback && replayStep && contestedSummary ? (
          <section className="onstage-grid">
            <div className="onstage-main">
              ...
              <StateTimeline steps={getPreparedMigrationStateSteps(replayStep.resolvedState)} />
...
function StateTimeline(props: { steps: StateStep[] }) {
  const { steps } = props;

  return (
    <div className="state-timeline">
      {steps.map((step, index) => (
        <div key={step.key} className={`state-timeline-step ${step.active ? "active" : ""} ${step.active ? step.tone : ""}`}>
          <div className="state-timeline-dot" />
          <span className="state-timeline-label">{step.label}</span>
          {index < steps.length - 1 ? <div className="state-timeline-connector" /> : null}
        </div>
      ))}
    </div>
  );
...
function StageSummaryCard(props: {
  ...
  return (
    <article className={`stage-card stage-card--summary stage-card--client ${summary.tone}`}>
      ...
      <h3>{summary.title}</h3>
      <p>{summary.detail}</p>
      {summary.clientAction ? (
        <div className="client-action-bar">
          <span className="mini-label">Client behavior</span>
          <strong>{summary.clientAction}</strong>
        </div>
      ) : null}
...
function StageEventStatusCard(props: {
  ...
  const { title, rows, rehearsal } = props;
  const observedLabel = rehearsal ? "resolved locally" : "seen on relays";
        ...
        {rows.map((row) => (
          <div key={row.id} className="stage-status-row">
            ...
            <span className={`pill ${row.status === "observed" ? "ok" : row.status === "sent" ? "warn" : "muted"}`}>
              {row.status === "observed" ? observedLabel : row.status === "sent" ? "sent" : "queued"}
            </span>
          </div>
        ))}
      </div>
      ...
      <aside className="console-shell operator-column">
        ...
        <article className="console-card">
          ...
          <div className="button-stack">
            ...
            >
              {notice.showAttestAction ? "Open Path C" : "Path C opens after execution"}
            </button>
          </div>
        </article>
      ...
      <aside className="console-shell operator-column">
        ...
        <article className="console-card">
          ...
          <div className="checklist">
            ...
            <div className={`check-item ${pmaSent ? "done" : ""}`}>Publish migration authority</div>
            <div className={`check-item ${proofSent ? "done" : ""}`}>Publish timestamp proof</div>
          </div>
          <div className="button-stack">
            <button
              className="button primary"
            ...
            <button
              ...
              onClick={() => {
                void (async () => {
                  ...
                  if (operator.demoMode === "live" && !operator.isConnected) {
                  ...
                  await operator.publishAllRemainingActions();
                  operator.setOperatorMessage(`Signup complete for ${signupHandle}. Authority and proof published.`);
                })().catch((error: unknown) =>
                  operator.setOperatorMessage(error instanceof Error ? error.message : String(error)),
                );
              }}
            >
              {proofSent ? "Signup already published" : "Finish signup and publish authority + proof"}
            </button>
          </div>
        </article>
      </aside>
                ...
                {step.visibleClaims.length === 0 ? (
                  ...
                  <ul className="compact-list">
                    {step.visibleClaims.map((event) => (
                      <li key={event.id}>
                        <strong>{event.pubkey === getSingleTagValue(event, "n") ? "New key claim" : "Old key claim"}</strong>
