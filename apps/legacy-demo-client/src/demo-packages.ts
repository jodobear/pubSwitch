...
function describePathAAction(event: NostrEvent): string {
  switch (event.kind) {
    case PMA_KIND:
      return "Migration authority (1776)";
    case PMU_KIND:
      return "Authority update (1779)";
    case PMX_KIND:
      return "Execution (1777)";
    case OTS_KIND:
      return "Timestamp proof (1040)";
    default:
...
function describePathCAction(event: NostrEvent, scenario: PathCFixtureScenario): string {
  if (event.kind === STC_KIND) {
    ...
    return role === "new" ? "Transition claim as new key" : "Transition claim as old key";
  }

  const stance = getSingleTagValue(event, "s") ?? "uncertain";
  if (event.pubkey === scenario.oldPubkey || event.pubkey === scenario.newPubkey) {
    return `Attestation (${stance}) self-authored`;
  }

  return `Attestation (${stance}) third party`;
}
