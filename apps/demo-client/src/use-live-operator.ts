...
export function useLiveOperator() {
  ...
  async function publishPreparedEvent(event: NostrEvent) {
    ...
    setOperatorMessage(`Published ${event.id ? `${event.id.slice(0, 10)}...${event.id.slice(-8)}` : "event"} to ${socketsRef.current.size} connected relays.`);
  }
  ...
  async function publishAllRemainingActions() {
    ...
    setOperatorMessage(`Published ${remaining.length} prepared event${remaining.length === 1 ? "" : "s"}.`);
  }
  ...
  async function publishLiveNote() {
    ...
    await publishEvent(event);
    setOperatorMessage(`Published kind 1 note ${event.id ? `${event.id.slice(0, 10)}...${event.id.slice(-8)}` : ""} as ${selectedNoteActor.label}.`);
  }
  ...
  function resetSession() {
    ...
    setPublishCursor(0);
    setSessionSince(Math.floor(Date.now() / 1000));
    setOperatorMessage("Started a fresh observer session and cleared local state.");
  }
